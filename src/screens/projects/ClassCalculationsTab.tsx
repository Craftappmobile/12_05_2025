/**
 * Класовий компонент вкладки розрахунків
 * 
...
interface ClassCalculationsTabProps {
  projectId: string;
  onRefresh?: () => void;
  navigation?: any; // Опціональний об'єкт навігації для направлення на інші екрани
}

interface ClassCalculationsTabState {
  loading: boolean;
  calculations: any[];
}

/**
 * Класовий компонент для відображення списку розрахунків
 */
class ClassCalculationsTab extends React.Component<ClassCalculationsTabProps, ClassCalculationsTabState> {
  // Важливо встановити displayName для компонента
  static displayName = 'ClassCalculationsTab';
  
  constructor(props: ClassCalculationsTabProps) {
    super(props);
    this.state = {
      loading: true,
      calculations: []
  ...
  navigateToCalculationDetails = (calculationId: string) => {
    if (this.props.navigation?.navigate) {
      this.props.navigation.navigate('CalculationDetails', { calculationId });
    } else {
  ...
  navigateToCalculatorsList = () => {
    if (this.props.navigation?.navigate) {
      this.props.navigation.navigate('CalculatorsList', { projectId: this.props.projectId });
    } else {
...
export default ClassCalculationsTab;