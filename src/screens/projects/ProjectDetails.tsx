...
import { useProjects, ProjectWithCalculations } from '../../hooks/useProjects';
import { database, projectsCollection } from '../../database';
...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ route, navigation }) => {
  // Захист від відсутніх параметрів
  if (!route?.params?.projectId) {
    console.warn('ProjectDetails: Відсутній projectId в параметрах');
    
    useEffect(() => {
      // Затримка, щоб користувач побачив повідомлення перед поверненням
  ...
  const [project, setProject] = useState<ProjectWithCalculations | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
  const [activeTab, setActiveTab] = useState<'info' | 'calculations' | 'notes' | 'photos'>('info');
  const { updateProject, deleteProject } = useProjects();

  // Завантаження деталей проекту
  const fetchProjectDetails = async () => { try {
      setLoading(true);
      ...
      const projectWithCalculations: ProjectWithCalculations = {
        id: projectRecord.id,
  ...
  const navigateToCalculationDetails = (calculationId: string) => {
            ...
            {project.calculations.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                <Button 
                  ...
                  onPress={() => navigation.navigate('CalculatorsList', { projectId: project.id })} 
