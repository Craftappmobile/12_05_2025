/**
 * Класовий компонент вкладки нотаток
 * 
...
/**
 * Класовий компонент для відображення та редагування нотаток
 */
class ClassNotesTab extends React.Component<ClassNotesTabProps, ClassNotesTabState> {
  // Важливо встановити displayName для компонента
  static displayName = 'ClassNotesTab';
  
  constructor(props: ClassNotesTabProps) {
    super(props);
    this.state = {
      loading: false,
      notesLoading: true,
  ...
  updateNote = async (noteId: string) => {
    ...
    try {
      this.setState({ loading: true });
      await database.action(async () => {
        const noteToUpdate = await database.collections.get('notes').find(noteId);
        ...
        { 
          ...
          onPress: async () => {
            try {
              this.setState({ loading: true });
              await database.action(async () => {
                const noteToDelete = await database.collections.get('notes').find(noteId);
  ...
  // Скасувати редагування/створення
  cancelEditing = () => {
    this.setState({
      editingNote: null,
      noteTitle: '',
  ...
  // Зміна заголовка нотатки
  handleNoteTitleChange = (text: string) => {
    this.setState({ noteTitle: text });
  };

  // Зміна вмісту нотатки
  handleNoteContentChange = (text: string) => {
    this.setState({ noteContent: text });
  };

  render() {
    const { loading, notesLoading, notes, noteContent, noteTitle, editingNote } = this.state;
            ...
            {(editingNote !== null || (noteTitle !== '' || noteContent !== '')) && (
              <View style={styles.noteForm}>
                <TextInput
                  ...
                  value={noteTitle}
                  onChangeText={this.handleNoteTitleChange}
                  placeholder="Назва нотатки"
                />
                <TextInput
                  style={styles.noteContentInput}
                  value={noteContent}
                  onChangeText={this.handleNoteContentChange}
