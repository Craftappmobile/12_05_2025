/**
 * Класовий компонент вкладки лічильника
 * 
 * Цей компонент використовує клас React.Component, 
 * що безпечно працює в Bridgeless режимі
 */
...
interface ClassCounterTabProps {
  projectId: string;
  onRefresh?: () => void;
  onProgressChange?: (progress: number) => void;
}

interface ClassCounterTabState {
  loading: boolean;
  projectData: any;
  currentRow: number;
  totalRows: number;
...
/**
 * Класовий компонент для відображення та керування лічильником
 */
class ClassCounterTab extends React.Component<ClassCounterTabProps, ClassCounterTabState> {
  // Важливо встановити displayName для компонента
  static displayName = 'ClassCounterTab';
  
  constructor(props: ClassCounterTabProps) {
    super(props);
    this.state = {
      loading: true,
      projectData: null,
  ...
  componentDidUpdate(prevProps: ClassCounterTabProps, prevState: ClassCounterTabState) {
    // Оновлення прогресу при зміні значень
    if (prevState.currentRow !== this.state.currentRow || prevState.totalRows !== this.state.totalRows) {
      if (this.state.totalRows > 0) {
        const progress = Math.min(Math.round((this.state.currentRow / this.state.totalRows) * 100), 100);
  ...
  updateProjectProgress = async (progress: number) => {
    try {
      await database.action(async () => {
        const projectToUpdate = await database.get('projects').find(this.props.projectId);
        await projectToUpdate.update(project => {
          project.progress = progress;
          project.currentRow = this.state.currentRow;
          project.totalRows = this.state.totalRows;
        });
      });
  ...
  incrementCounter = (amount: number) => {
    const newValue = this.state.currentRow + amount;
    this.setState({
      currentRow: newValue <= this.state.totalRows ? newValue : this.state.totalRows
    });
  };
  ...
  handleCurrentRowChange = (text: string) => {
    const value = parseInt(text) || 0;
    this.setState({
      currentRow: value >= 0 ? (value <= this.state.totalRows ? value : this.state.totalRows) : 0
    });
  };
...
export default ClassCounterTab;